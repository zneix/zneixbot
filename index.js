...
(async function(){
    ...
    load.gracefulExits(); //SIGINT and process.exit defs for graceful shutdowns
    await client.login(require('./src/json/auth').token).catch(err => {console.error(err);process.emit('SIGINT');}); //logging in to Discord
})();
