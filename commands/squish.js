...
exports.run = async message => {
    ...
    if (message.attachments.size){
        ...
        if (res) url = `https://cdn.discordapp.com/emojis/${res[3]}.${res[1].length ? 'gif' : 'png'}`;
        ...
        //user avatars (mention or id)
            ...
            if (!message.mentions.members.first().user.avatarURL()) throw ['normal', 'Target user doesn\'t have an avatar.'];
            url = message.mentions.members.first().user.avatarURL({format:'png', dynamic:true});
        }
    ...
    try {
        ...
        let sq = canvas.createCanvas(img.width, Math.floor(img.height / 3));
        ...
        await message.channel.send('Squished image', {files: [sq.toBuffer()] });
        msg.delete();
    }
