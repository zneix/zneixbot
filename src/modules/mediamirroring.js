module.exports = async message => {
    ...
    const mirrorChannelID = client.go[message.guild.id].config.modules.logging.mediamirror;
    if (!mirrorChannelID) return;
    const mirrorChannel = message.guild.channels.cache.get(mirrorChannelID);
    ...
    const mirroredMedia = await Promise.all([...message.attachments.keys()]
    ...
    const mirrorMessage = await mirrorChannel.send({embed: embed, files: mirroredMedia.map(media => { return {attachment: media.buffer, name: `mirrored_${media.name}`} })});
