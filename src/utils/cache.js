...
exports.getUserFromMessage = async function(message, { useQuery, withinGuild } = {}){
    ...
    const userFromID = client.users.cache.get(message.args[0]);
    ...
    if (message.guild && useQuery){
        const queryResult = await message.guild.members.fetch({query: message.args.join(' '), limit: 1});
...
exports.getRoleFromMessage = function(message, { useQuery } = {}){
    ...
    //roleID
    const roleFromID = message.guild.roles.cache.get(message.args[0]);
    if (roleFromID) return roleFromID;
    if (useQuery){
        //fuzzy search if previous methods didn't work (also filtering out default server role)
        const Fuse = require('fuse.js');
        const fuse = new Fuse(message.guild.roles.cache.filter(r => r.position != 0).map(r => {
            return {
                name: r.name,
                id: r.id
            }
        ...
        const res = fuse.search(message.args.join(' '));
